...
- **核心依賴**：
  ...
  - `pytest`：單元測試與整合測試
- **SDK**：`sdk/ros_robot_controller_sdk.py`（作為標準 Python package `sdk` 導入）
...
**注意**：`nexarm_sdk_path` 已棄用。`robot_agent2.py` 現直接使用 `from sdk.ros_robot_controller_sdk import Board` 導入，無需動態修改 `sys.path`。
...
- 修改 `sdk/ros_robot_controller_sdk.py` 時需極度謹慎，該文件直接控制硬體通訊協議
