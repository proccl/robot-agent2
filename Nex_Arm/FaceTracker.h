...
private:
    ...
    float _target_x = 200, _target_y = 0, _target_z = 200;

    unsigned long _ignore_data_until = 0;
    unsigned long _last_i2c_time = 0;

    FT_State _currentState = FT_IDLE;
    ...
    // Mailbox v2 helpers
    bool     initMailbox();
    SlotMeta readSlotMeta(uint16_t offset);
    void     writeSlotMeta(uint16_t offset, const SlotMeta& m);
    ...
    void handleFaceKp(const uint8_t* payload, uint16_t plen);
